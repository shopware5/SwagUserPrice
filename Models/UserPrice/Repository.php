...
class Repository extends ModelRepository
{
    ...
    public function getCustomersQueryBuilder($filter, $sort)
    {
        ...
        )->from('Shopware\Models\Customer\Customer', 'customer')->join('customer.billing', 'billing')->leftJoin(
            'customer.attribute',
            'attribute'
        );

        if (!empty($filter)) {
            $builder->andWhere('billing.number LIKE ?1')->orWhere('billing.firstName LIKE ?2')->orWhere(
                'billing.lastName LIKE ?2'
            )->orWhere('customer.email LIKE ?2')->orWhere('customer.groupKey LIKE ?2')->orWhere(
                'billing.company LIKE ?2'
            )->orWhere('billing.city LIKE ?2')->orWhere('billing.zipCode LIKE ?1')->setParameter(
                1,
                $filter . '%'
    ...
    public function getArticlesQueryBuilder($filter, $start, $limit, $sort, $main, $groupId)
    {
        ...
        if (!empty($filter)) {
            $builder->andWhere('article.name LIKE :filter')->orWhere('detail.ordernumber LIKE :filter')->setParameter(
                'filter',
                '%' . $filter . '%'
            );
        }
        ...
        if ($sort != null) {
            $builder->orderBy($sort[0]['property'], $sort[0]['direction']);
        }

        $builder->addOrderBy('article.id', 'ASC')->addOrderBy('detail.ordernumber', 'ASC');
    ...
    public function getPricesQueryBuilder($detailId, $groupId)
    {
        ...
        )->from('Shopware\CustomModels\UserPrice\Price', 'prices')->where('prices.priceGroupId = ?1')->andWhere(
            'prices.articleDetailsId = ?2'
        )->setParameter(1, $groupId)->setParameter(2, $detailId)->orderBy('prices.from', 'ASC');
